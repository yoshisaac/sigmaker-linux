...
std::string BuildByteArrayWithMaskSignatureString(const Signature &signature) {
  ...
  for (const auto &byte : signature) {
    pattern << "\\x"
            << std::format("{:02X}", (byte.isWildcard ? 0 : byte.value));
...
void AddBytesToSignature(Signature &signature, ea_t address, size_t count,
                         bool wildcard) {
  // signature.reserve( signature.size() + count ); // Not sure if this is
  // overhead for average signature creation
...
void TrimSignature(Signature &signature) {
  auto it = std::find_if(signature.rbegin(), signature.rend(),
                         [](const auto &sb) { return !sb.isWildcard; });
