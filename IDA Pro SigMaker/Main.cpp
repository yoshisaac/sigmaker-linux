  ...
  while (true) {
    ...
    if (sigPartLength > maxSignatureLength) {
      if (askLongerSignature) {
        auto result = ask_yn(
            ASKBTN_YES, "Signature is already at %" PRIu64 " bytes. Continue?",
            signature.size());
        if (result == 1) { // Yes
          sigPartLength = 0;
        } else if (result == 0) { // No
...
void PrintSignatureForEA(const std::expected<Signature, std::string> &signature,
  ...
  msg("Signature for %" PRIx64 ": %s\n", (uint64_t)ea, signatureStr.c_str());
...
static void FindXRefs(ea_t ea, bool wildcardOperands,
  ...
  for (auto xref_ok = xref.first_to(ea, XREF_FAR); xref_ok;
    ...
    replace_wait_box(
        "Processing xref %" PRIu64 " of %" PRIu64 " (%0.1f%%)...\n\n"
        "Suitable Signatures: %" PRIu64 "\n"
        "Shortest Signature: %" PRIu64 " Bytes",
        (uint64_t)(i + 1), (uint64_t)xrefCount, (float(i) / xrefCount) * 100.0f,
        (uint64_t)xrefSignatures.size(), (uint64_t)shortestSignatureLength);
  ...
  if (!stringMask.empty()) {
    ...
    if (GetRegexMatches(input, std::regex(R"(\\x(?:[0-9A-F]{2}))"),
      ...
      for (size_t i = 0; const auto &m : rawByteStrings) {
        SignatureByte b{std::stoi(m.substr(2), nullptr, 16),
                        stringMask[i++] == '?'};
